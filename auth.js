...
// Настройки маскота (вид/язык/вкл-выкл) — приходят из того же /check,
// маскот на menu.html читает их из localStorage вместо отдельного
// запроса, аналогично display_name выше. Если поля нет в ответе
// (старый auth-worker без этой фичи задеплоен) — просто не пишем,
// mascotInit() в menu.html сам подставит дефолт.
if (checkData.mascotSettings) {
localStorage.setItem("mascot_settings", JSON.stringify(checkData.mascotSettings));
}
}
} catch(e) {
...
localStorage.removeItem("test_result");
localStorage.removeItem("test_mistakes");
localStorage.removeItem("mascot_settings");
